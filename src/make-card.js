 ...
 * @param {boolean} noControls - наличие контролов у карточки
 * @return {string} - возвращает строку с шаблоном карточки
 */
export default (noControls) => {
  return `
  <article class="film-card ${noControls ? `film-card--no-controls` : ``}">
    ...
    ${noControls ? `` : `<p class="film-card__description">A priest with a haunted past and a novice on the threshold of her final vows are sent by the Vatican to investigate the death of a young nun in Romania and confront a malevolent force in the form of a demonic nun.</p>`}
